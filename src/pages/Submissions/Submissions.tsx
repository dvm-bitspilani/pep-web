...
const Submissions: React.FC = () => {
  // Deadlines
  const DEADLINE_ABSTRACT = new Date("2026-04-05T23:59:59");
  const DEADLINE_PAPER = new Date("2026-04-05T23:59:59");
  ...
  const handleAbstractSubmit = (e: React.FormEvent) => {
    ...
    axios
      .post(
        `${API_BASE_URL}/registrations/abstract_submission/`,
        abstractFormData,
      )
      .then((response) => {
        if (response.status === 200 || response.status === 201) {
          ...
          setAbstractError(
            response.data.error || response.data.message || "Submission failed",
          );
        }
      })
      .catch((error) => {
          ...
          <div className={styles.submissionColumn}>
            ...
            >
              {isAbstractOpen ? (
                <>
                  {abstractError && (
                    <div className={styles.error}>{abstractError}</div>
                  )}
                  {abstractSuccess && (
                    <div className={styles.success}>{abstractSuccess}</div>
                  )}
                  {!abstractSuccess && (
                    <div className={styles.info}>
                      <strong>Deadline: 5th April 2026</strong>
                      <p>
                        Please ensure you have registered before submitting your
                        abstract.
                      </p>
                    </div>
                  )}
          ...
          <div className={styles.submissionColumn}>
            ...
            >
              {isPaperOpen ? (
                <>
                  {paperError && (
                    <div className={styles.error}>{paperError}</div>
                  )}
                  {paperSuccess && (
                    <div className={styles.success}>{paperSuccess}</div>
                  )}
                  {!paperSuccess && (
                    <div className={styles.info}>
                      <strong>Deadline: 5th April 2026</strong>
                      <p>
                        Abstract submission must be cleared before submitting
                        your paper.
                      </p>
                    </div>
                  )}
